...
def parse_telegram_url(url: str):
    ...
    if not msg_id_str.isdigit():
        raise ValueError("Could not parse message ID.")
    msg_id = int(msg_id_str)
    
    chat_id_str = parts[-2]
    # Handle private link format: /c/12345/678
...
async def pin_specific_message(req: DeleteMsgRequest):
...
async def unpin_specific_message(req: DeleteMsgRequest):
...
@app.post("/api/mood")
async def set_mood(data: MoodUpdate):
    try:
